...
//------------------------------------------------------------admin----------------------------------
  ...
  Future<void> buatPaketSoal(Map<String, dynamic> data) async {
    return ref
        ...
        .push()
        .set(data)
        .then((_) => print("buat paket soal sukses, cus input"));
  }
  ...
  Future<void> pushPaketSoal(
      String kelas, String mapel, CariSoalModel sual) async {
    return ref
        .child('carisoal/$kelas/$mapel/${sual.idsoalnya}')
        .set(sual.toMap())
        .then((_) => print("push soal sukses"))
        .then((_) {
      return ref
          .child('banksoal/${sual.idsoalnya}/published')
          .set(true)
          .then((_) => print('change published to true'));
    });
  }

  Future<void> unPublishSoal(
      String kelas, String mapel, String idpush, String idsoalnya) async {
    return ref.child('carisoal/$kelas/$mapel/$idpush').remove().then((_) {
      return ref
          .child('banksoal/$idsoalnya/published')
          .set(false)
